...
{
    struct http_cancel_request_params params = { .id = id };
    ULONG ret = ERROR_SUCCESS;
    ULONG local_ret_size;
    OVERLAPPED sync_ovl;
    ...
    if (!DeviceIoControl(queue, IOCTL_HTTP_CANCEL_REQUEST, &params, sizeof(params), NULL, 0, &local_ret_size, ovl))
        ret = GetLastError();
    ...
    if (ovl == &sync_ovl)
    {
        ...
        {
            ret = ERROR_SUCCESS;
            if (!GetOverlappedResult(queue, ovl, &local_ret_size, TRUE))
                ret = GetLastError();
        }
        CloseHandle(sync_ovl.hEvent);
    }
