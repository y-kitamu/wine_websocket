...
/* (msvcrt ヘッダーが strdup/_memicmp を提供するのでスタブ不要) */
...
struct connection
{
    ...
    /* WebSocket state: if TRUE this connection was upgraded (101 sent) and
     * the http daemon must NOT parse incoming data as HTTP. */
    bool is_websocket;
    LIST_ENTRY body_irp_queue; /* pending HttpReceiveRequestEntityBody IRPs */
...
static void accept_connection(SOCKET socket)
{
    ...
    InitializeListHead(&conn->body_irp_queue);
...
static void close_connection(struct connection *conn)
{
    ...
    /* Cancel any pending WebSocket body IRPs. */
    while (!IsListEmpty(&conn->body_irp_queue))
    {
        IRP *irp = CONTAINING_RECORD(RemoveHeadList(&conn->body_irp_queue), IRP, Tail.Overlay.ListEntry);
...
static BOOL host_matches(const struct url *url, const char *conn_host)
{
    ...
    else
    {
        ...
        if (!_memicmp(url->url + 7, conn_host, host_len))
            return TRUE;
    }
...
static void receive_data(struct connection *conn)
{
    ...
    /* WebSocket mode: buffer raw data and complete any pending body IRPs. */
    if (conn->is_websocket)
    {
        conn->content_len = conn->len; /* expose all buffered bytes as entity body */
        /* Complete pending HttpReceiveRequestEntityBody IRPs. */
        while (!IsListEmpty(&conn->body_irp_queue))
        {
            IRP *body_irp;
            IO_STACK_LOCATION *body_stack;
            DWORD out_len;
            ULONG copy_len;

            body_irp   = CONTAINING_RECORD(conn->body_irp_queue.Flink, IRP, Tail.Overlay.ListEntry);
            body_stack = IoGetCurrentIrpStackLocation(body_irp);
            out_len    = body_stack->Parameters.DeviceIoControl.OutputBufferLength;
            copy_len   = min(conn->content_len, out_len);

            if (!copy_len) break; /* no data yet */

            RemoveHeadList(&conn->body_irp_queue);
            IoSetCancelRoutine(body_irp, NULL);

            memcpy(body_irp->AssociatedIrp.SystemBuffer, conn->buffer, copy_len);
            memmove(conn->buffer, conn->buffer + copy_len, conn->len - copy_len);
            conn->len          -= copy_len;
            conn->content_len  -= copy_len;

            body_irp->IoStatus.Information = copy_len;
            body_irp->IoStatus.Status = STATUS_SUCCESS;
            IoCompleteRequest(body_irp, IO_NO_INCREMENT);
        }
        return;
    }
...
static void WINAPI http_receive_body_cancel(DEVICE_OBJECT *device, IRP *irp)
{
    TRACE("device %p, irp %p (body).\n", device, irp);
    IoReleaseCancelSpinLock(irp->CancelIrql);
    EnterCriticalSection(&http_cs);
    RemoveEntryList(&irp->Tail.Overlay.ListEntry);
    LeaveCriticalSection(&http_cs);
...
static NTSTATUS http_send_response(struct request_queue *queue, IRP *irp)
{
    ...
    if ((conn = get_connection(response->id)))
    {
        ...
        {
            /* If this is a 101 Switching Protocols response, transition to WebSocket mode.
             * Do NOT re-enter the HTTP parsing loop; leave the connection for raw I/O.
             * Keep conn->req_id so subsequent HttpSendResponseEntityBody / HttpReceiveRequestEntityBody
             * calls can find this connection via get_connection(). */
            if (response->len >= 12 && !memcmp(response->buffer, "HTTP/1.1 101", 12))
            {
                TRACE("WebSocket upgrade detected (101); switching connection to WS mode.\n");
                conn->is_websocket = true;
                conn->queue = NULL;
                /* Do NOT reset req_id – callers use it to address subsequent entity body I/O. */
                conn->available = FALSE;
                /* Keep FD_READ so receive_data() buffers incoming WS frames. */
                WSAEventSelect(conn->socket, request_event, FD_READ | FD_CLOSE);
            }
            else
            {
            /* WebSocket entity-body send: just send; do not touch HTTP state. */
            if (!conn->is_websocket)
            {
...
static NTSTATUS http_receive_body(struct request_queue *queue, IRP *irp)
{
    ...
    if ((conn = get_connection(params->id)))
    {
        ...
        {
            /* WebSocket: no data yet – pend the IRP; receive_data() will complete it. */
            TRACE("WebSocket body recv: no data yet, pending IRP.\n");
            IoSetCancelRoutine(irp, http_receive_body_cancel);
            if (irp->Cancel && IoSetCancelRoutine(irp, NULL))
            {
                ret = STATUS_CANCELLED;
            }
            ...
            {
                IoMarkIrpPending(irp);
                InsertTailList(&conn->body_irp_queue, &irp->Tail.Overlay.ListEntry);
                ret = STATUS_PENDING;
            }
        }
        else
...
/* Cancel any pending WebSocket body-receive IRP for the connection identified
 * by params->id. Mirrors real HttpCancelHttpRequest, which Wine never
 * implemented (httpapi.spec listed it as "stub"), causing the whole process
 * to abort when kabu STATION's .NET runtime called it after a WebSocket send. */
static NTSTATUS http_cancel_request(struct request_queue *queue, IRP *irp)
{
    ...
    if ((conn = get_connection(params->id)))
    {
        while (!IsListEmpty(&conn->body_irp_queue))
        {
            IRP *body_irp = CONTAINING_RECORD(RemoveHeadList(&conn->body_irp_queue), IRP, Tail.Overlay.ListEntry);
            IoSetCancelRoutine(body_irp, NULL);
            body_irp->IoStatus.Status = STATUS_CANCELLED;
            body_irp->IoStatus.Information = 0;
            IoCompleteRequest(body_irp, IO_NO_INCREMENT);
        }
    }
